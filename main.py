...
def main(_):
    ...
    ax.set_xlabel('epoch') ; ax.set_ylabel('GAN Loss')

    def plt_dynamic(x, y, y_1, ax, ticks,title, colors=['b']):
        ax.plot(x, y, 'b', label="discrminator loss")
        ax.plot(x, y_1, 'r', label="generator loss")
        if len(x)==1:
            plt.legend()
            plt.title(title)
        plt.yticks(ticks)
    ...
    for epoch in range(FLAGS.epoch):
        ...
        for batch_images in iterate_minibatches(FLAGS.batch_size):
            ...
            ## Visualizing layers of Discriminator
            for _ in range(2):
            ...
            if np.mod(iter_counter, FLAGS.sample_step) == 0:
                ...
                plt_dynamic(time_stamp, discriminator_loss,generator_loss, ax, np.arange(1.3, 1.8, step=0.04),'generator discminator loss curve') 
    ...
    is_success = activation_visualization(sess_graph_path = sess, value_feed_dict = {z: batch_z, real_images: batch_images}, 
                                          layers=layers, path_logdir=os.path.join("Log","discriminator"),
                                          path_outdir=os.path.join("Output","discriminator"))
